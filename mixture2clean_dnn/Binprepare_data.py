...
def create_mixture_csv(args):
    ...
    for speech_na in speech_names:
        ...
        if data_type == 'train':
            #它这里是随机取出的噪声数据，要把它改成每个语音数据和每种噪声数据都做加性噪声处理
            #并且加入不同信噪比的加性噪声
...
def calculate_mixture_features(args):
    ...
    noise_dir = args.noise_dir
    data_type = args.data_type
    snr_jj = [20, 15, 10, 5, 0, -5]#args.snr
    fs = cfg.sample_rate
...
def pack_features(args):
    ...
    workspace = args.workspace
    data_type = args.data_type
    snr_jk = [20, 15, 10, 5, 0, -5]#args.snr
...
def compute_scaler(args):
    ...
    workspace = args.workspace
    data_type = args.data_type
    snr_jl = [20, 15, 10, 5, 0, -5]#args.snr
...
#train --workspace G:\研二上学期\paper\denoise\coding\BinCode\sednn\mixture2clean_dnn\BinCreate --tr_snr 5 --te_snr 5 --lr 0.0001